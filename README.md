...
llama.cpp/
  ...
  enc.env                         # ^ encrypted, committed (holds no secret)
mcpjungle/
...
Anything marked *gitignored* has a committed `.example` sibling. Every `.env` above
except the root one is generated by `./runit.sh` from the `enc.env` next to it.
...
Each service's secrets live in a committed, age-encrypted `<service>/enc.env`.
`./runit.sh` decrypts each one into the `<service>/.env` beside it — Compose needs
a plain file for both `env_file:` and `${VAR}` interpolation — and then runs
`docker compose`. With the age key in place, setup is one command:

```sh
./runit.sh --decrypt-only   # writes the six .env files, starts nothing
```
...
is no second copy. Starting from scratch instead, build the files from the
examples and skip to step 2:
...
- ~~**`open-webui/.env` mixes two syntaxes.**~~ Fixed. The four web-search lines
...
./runit.sh              # push the change into hermes/.env and restart
```
